...
def eval_checkpoint_over_block_length_range(result_dir_path, model, args, val_loader, num_iters, block_range_iter, length_range_iter, loss_label="MSE"):
    ...
    if result_dir_path:
        results_df.to_csv(os.path.join(result_dir_path, "val_block_length_results.csv"))
    return results_df
...
def get_loss_on_video_batch(data_np: np.ndarray,
                            ...
                            num_blocks=None,
                            length=None,
                            loss_label=None, #MSE, Weight, Activity
                            zero_grad=True
                            ):
    """
    ...
    Args: 
    """
    data = torch.tensor(data_np, dtype=torch.float32).to(model.device)
    ...
    if loss_label != None:
        if loss_label == "MSE":
            return MSE_total / num_timesteps
        elif loss_label == "Weight":
            return weight_reg_total / num_timesteps
        elif loss_label == "Activity":
            return activity_reg_total / num_timesteps
...
if __name__ == "__main__":
    ...
    for data_np in video_data_generator(video_paths, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS, rescale=[VIDEO_HEIGHT, VIDEO_WIDTH], float01=True): 
        ...
        loss = get_loss_on_video_batch(data_np, args, model, args.num_steps_per_frame, WEIGHT_REGULARIZATION, ACTIVITY_REGULARIZATION, instrument_correlations=False) 
        ...
        if epoch % args.val_period == 0:
            ...
            with torch.no_grad(): 
                ...
                for data_np_val in tqdm(video_data_generator(val_paths, 
                    ...
                    model.Phi = []
                    model.correlation_tensor = []
                    val_loss = get_loss_on_video_batch(data_np_val, args, model, args.num_steps_per_frame, WEIGHT_REGULARIZATION, ACTIVITY_REGULARIZATION)
                    total_val_loss += val_loss.item()
                    cnt += 1
